package com.ribbontek.shared.specdsl

import com.ribbontek.shared.context.AbstractIntegTest
import com.ribbontek.shared.repository.StarRating
import com.ribbontek.shared.repository.TvShow
import com.ribbontek.shared.repository.TvShowRepository
import com.ribbontek.shared.repository.availableOnNetflix
import com.ribbontek.shared.repository.hasKeyword
import com.ribbontek.shared.repository.hasKeywordIn
import com.ribbontek.shared.repository.hasName
import com.ribbontek.shared.repository.hasReleaseDateIn
...
/**
 * Credit to https://github.com/consoleau/kotlin-jpa-specification-dsl
 */
@Transactional
class JPASpecificationDSLTest : AbstractIntegTest() {
    @Autowired
    ...
    @BeforeAll
    fun beforeAll() {
        executeNativeQueryUpdate(setUpTableScript)
    }

    @AfterAll
    fun afterAll() {
        executeNativeQueryUpdate(tearDownTableScript)
    }

    private fun executeNativeQueryUpdate(queryString: String) {
     ...
     */
    fun TvShowQuery.toSpecification(): Specification<TvShow> =
        and(
            hasName(name),
     ...
     */
    fun Iterable<TvShowQuery>.toSpecification(): Specification<TvShow> =
        or(
    ...
    companion object {
        private val setUpTableScript =
            """
            CREATE TABLE IF NOT EXISTS star_rating (
                id SERIAL PRIMARY KEY,
                stars INT NOT NULL
            );
            CREATE SEQUENCE IF NOT EXISTS star_rating_seq START 1;
            CREATE TABLE IF NOT EXISTS tv_show (
                id SERIAL PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
                synopsis TEXT,
                available_on_netflix BOOLEAN,
                release_date TEXT
            );
            CREATE SEQUENCE IF NOT EXISTS tv_show_seq START 1;
            CREATE TABLE IF NOT EXISTS tv_show_star_ratings (
                id SERIAL PRIMARY KEY, 
                tv_show_id INT,
                star_ratings_id INT,
                CONSTRAINT fk_tv_show FOREIGN KEY (tv_show_id) REFERENCES tv_show(id),
                CONSTRAINT fk_star_rating FOREIGN KEY (star_ratings_id) REFERENCES star_rating(id)
            );
            CREATE SEQUENCE IF NOT EXISTS tv_show_star_ratings_seq START 1;
            """.trimIndent()

        private val tearDownTableScript =
            """
            ALTER TABLE tv_show_star_ratings DROP CONSTRAINT IF EXISTS fk_tv_show;
            ALTER TABLE tv_show_star_ratings DROP CONSTRAINT IF EXISTS fk_star_rating;
            DROP TABLE IF EXISTS tv_show_star_ratings;
            DROP SEQUENCE IF EXISTS tv_show_star_ratings_seq;
            DROP TABLE IF EXISTS tv_show;
            DROP SEQUENCE IF EXISTS tv_show_seq;
            DROP TABLE IF EXISTS star_rating;
            DROP SEQUENCE IF EXISTS star_rating_seq;
            """.trimIndent()
    }
}
